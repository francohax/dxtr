      ...
      <div className="animate-fade-in relative w-full max-w-md overflow-hidden rounded-2xl border border-zinc-700/60 bg-zinc-900 shadow-2xl">
        <div className="flex items-center justify-between border-b border-zinc-800/60 px-4 py-3">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold text-zinc-200">Select Move</span>
          </div>
          <button
            onClick={onClose}
              ...
              return (
                <li key={name}>
                  <button
                    ...
                    className={`group flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm transition hover:bg-zinc-800 ${i === activeIndex ? "bg-zinc-800" : ""
                      }`}
                  >
                    {summary?.type
...
export function MoveFuzzySearch({ moveNames, value, onSelect, onClear, inputRef, isLoadingMove, attackerSprite, attackerName, openModalRef, attackingOnly, placeholder }: MoveFuzzySearchProps) {
  const [query, setQuery] = useState("");
    ...
    return (
      <div className="">
        <div
          ...
          style={{ "--type-glow": `var(--color-type-${value.type})`, "backgroundColor": `color-mix(in srgb, var(--color-type-${value.type}) 15%, black 80%)` } as React.CSSProperties}
          className="min-w-60 w-full group relative cursor-pointer rounded-xl border border-zinc-800 bg-zinc-900 p-3 shadow-[0_0_14px_color-mix(in_srgb,var(--type-glow)_20%,transparent)] transition hover:border-zinc-600 hover:bg-zinc-800/40 hover:shadow-[0_0_22px_color-mix(in_srgb,var(--type-glow)_35%,transparent)]"
        >

          <small className="absolute bottom-1 right-1 rounded-md bg-zinc-800 px-1 py-0.5 text-[8px] font-medium uppercase tracking-wide text-zinc-500">
            {value.category}
          </small>
          ...
          <div className="flex justify-between items-center gap-2.5 pr-14">
